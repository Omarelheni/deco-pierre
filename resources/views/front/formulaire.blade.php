...
<div class="popup">
    ...
    <section class="u-align-center u-clearfix u-image u-shading u-section-2" src="" data-image-width="1166" data-image-height="1600" id="sec-8e81">
        <div class="u-align-center u-clearfix u-sheet u-valign-middle u-sheet-1">
            <h2 class="u-text u-text-1">Merci pour votre achat!</h2>
            <p class="u-text u-text-2">Votre commande a été bien passée et nous vous contacterons au plus tot possible.</p>
            <a href="/accueil" class="u-align-center ">Retour a l'accueil</a>
        </div>
    </section>
      ...
      <div class="u-border-no-bottom u-border-no-left u-border-no-right u-border-no-top u-container-style u-expanded-width u-group u-shape-rectangle u-group-1">
        <div class="u-container-layout u-container-layout-1">
          <h3 class="u-align-center u-text u-text-1"><b>
              <span class="u-text-palette-3-light-2">COMM</span></b>ANDE
          </h3>
        </div>
      </div>
    </section>

    <section class="u-align-center u-clearfix u-section-3" id="sec-3495">
      <div class="u-clearfix u-sheet u-sheet-1">
        <p class="u-align-center u-text u-text-palette-3-light-1 u-text-1">Veuillez remplir ce formulaire pour passer votre commande.</p>
        <div class="u-form u-form-1">
          <form action="/commande" method="POST" class="u-clearfix u-form-spacing-15 u-form-vertical u-inner-form" style="padding: 15px;" id="contactForm">
           @csrf
            <div class="u-form-group u-form-name u-form-group-1">
              <label for="name-6797" class="u-form-control-hidden u-label">Name</label>
              <input type="text" placeholder="Nom" id="name-6797" name="nom" class="u-border-1 u-border-grey-30 u-input u-input-rectangle" required>
                <p style="color: red; display: none"></p>

            </div>
            <div class="u-form-group u-form-name u-form-group-2">
              <label for="name-6aa6" class="u-form-control-hidden u-label"></label>
              <input type="text" placeholder="Prénom" id="name-6aa6" name="prenom" class="u-border-1 u-border-grey-30 u-input u-input-rectangle" required>
                <p style="color: red; display: none"></p>
            ...
            <div class="u-form-address u-form-group u-form-group-4">
              <label for="address-7b19" class="u-form-control-hidden u-label"></label>
              <input type="text" placeholder="Adresse" id="address-7b19" name="adresse" class="u-border-1 u-border-grey-30 u-input u-input-rectangle" required>
                <p style="color: red; display: none"></p>

            </div>
            <div class="u-form-group u-form-phone u-form-group-5">
              <label for="phone-b013" class="u-form-control-hidden u-label"></label>
              <input type="tel"  placeholder="Téléphone" id="phone-b013" name="telephone" class="u-border-1 u-border-grey-30 u-input u-input-rectangle" required>

            </div>
                      <p style="color:red" id="errorform"></p>
...
<script>
    ...
    $('#contactForm').on('submit',function(e) {
        ...
        if (!pattern_tel.test(telephone.val())){
            test=false;
            var error =   telephone.next() ;
            error.css('display','inline');
            error.html('votre numero n\' est pas valide');
        }
        if ($.isNumeric(nom.val())){
            test=false;
            var error =   nom.next() ;
            error.css('display','inline');
            error.html('vote nom n\' est pas valide');
        }
        if ($.isNumeric(prenom.val())){
            test=false;
            var error =   prenom.next() ;
            error.css('display','inline');
            error.html('vote prenom n\' est pas valide');
        }
        var nb = JSON.parse("{{ json_encode(session('cart') ? count(session('cart')) : 0) }}");
        if(nb==0) {
            console.log($('#errorform'));
            $('#errorform').html('Veulliez ajouter des produits au panier');
            test = false ;
        }
        if (test) {
            $(this).submit(function (res) {
    $('#popup1').css('visibility', 'visible');
    $("#popup1").fadeTo("slow", 1);
